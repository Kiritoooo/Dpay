...
class Templates extends Addons
{
    ...
    {
        $menu = [];
        $config_file = ADDON_PATH . "templates" . DS . 'config' . DS . "menu.php";
    ...
    public function moduleInit($request)
    {
        ...
        if ($module && !in_array($module, $ignore_dir)) {
            ...
            if (($preview && $template) || $cache_value) {
                ...
                \think\Config::set('template.view_base', ".." . DS . "public" . DS . "templates" . DS . "modules" . DS . $template . DS);
                ...
                //加载语言包
                Lang::load(".." . DS . "public" . DS . "templates" . DS . "modules" . DS . $template . DS.'langs-'. \request()->langset() . '.ini' );
            } else {
                ...
                if ($module_template) {
                    ...
                    \think\Config::set('templates', $templates_config);
                    //加载语言包
                    Lang::load($self_template[$module_template]['view_base'].DS.'langs-'. \request()->langset() . '.ini' );
                }
            }
    ...
    public function addonModuleInit($request)
    {
        ...
        if (($preview && $template) || $cache_value) {
            ...
            $template = $template ? $template : $cache_value;
            \think\Config::set('template.view_base', ".." . DS . "public" . DS . "templates" . DS . "addons" . DS . $template . DS . $addon . DS);
            cache($cache_name, $template, 600);//缓存模板 预览10分钟
            ...
            \think\Config::set('templates', $templates_config);
            //加载语言包
            Lang::load(".." . DS . "public" . DS . "templates" . DS . "addons" . DS . $template . DS.'langs-'. \request()->langset() . '.ini' );
            ...
            if ($addon_template) {
                \think\Config::set('template.view_base', ($self_template[$addon_template]['view_base'] . DS . $addon . DS));
                $service = new Service();
                $templates_config = $service->getConfigKeyVal($self_template[$addon_template]['name'], 'addons');
                \think\Config::set('templates', $templates_config);
                //加载语言包
                Lang::load($self_template[$addon_template]['view_base'].DS.'langs-'. \request()->langset() . '.ini' );
            }
