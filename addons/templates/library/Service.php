...
class Service
{
    ...
    public function create($data)
    {
        ...
        $new_dir = ROOT_PATH . 'public' . DS . 'templates' . DS . $temp_name . DS . $data['name'] . DS;
        if (is_dir($new_dir)) {
            $this->error = ("{$data['name']}模板标识已经存在");
            return false;
        }
        //创建件静态资源文件夹
        $public_dir = ROOT_PATH . 'public' . DS . 'templates' . DS . "assets" . DS . $temp_name . DS . $data['name'] . DS;
    ...
    public function getConfig($name, $type = "addons")
    {
        ...
        //获取模板配置信息
        $config_file = ROOT_PATH . "public" . DS . "templates" . DS . $type . DS . $name . DS . 'config.ini';
        $config = [];
    ...
    {
        return ADDON_PATH . "templates" . DS . 'library' . DS . 'tp' . DS . $name;
    }
     ...
     * @return array
     */
    public function getLangsConfig($name, $type = "addons")
    {
        //获取模板配置信息
        $config_file = ROOT_PATH . "public" . DS . "templates" . DS . $type . DS . $name . DS . 'langs.ini';
        $config = [];
        if (is_file($config_file)) {
            $config = include $config_file;
        } else {
