...
class Templates extends Backend
{
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            if ($service->create($data)) {
                ...
                $this->error("失败:" . $service->getError());
            }
    ...
    public function langs($name = NULL)
    {
        ...
        if (!$name) $this->error('模板不存在');
        $service = new  Service();
        $config = $service->getConfigKeyVal($name, $temp_name);
        $lists = isset($config['langs']) ? $config['langs'] : [];
        return $this->view->fetch('', ['lists' => $lists, 'is_module' => $is_module]);
     ...
     * 添加语言
     */
    public function langsadd($name = NULL)
    {
        $is_module = $this->request->param('module', 0, 'intval');//1是模块0是插件
        $temp_name = $is_module == 1 ? "modules" : "addons";
        if (!$name) $this->error('模板不存在');
        if ($this->request->isPost()) {
            $params = $this->request->post("langs");
            $langsset = $this->request->post("langsset/a");
            if (!$langsset) $this->error("请输入语言信息");

            $langsname = $langsset[0]['value'];
            //保存语言配置文件
            $langsset = [$langsset[0]['key'] => $langsset[0]['value']];
            $service = new  Service();
            $config = $service->getConfig($name, $temp_name);
            $config_langs = array();
            foreach ($config as $k => &$row) {
                if ($row['name'] == 'langs') {
                    $row['value'] = array_merge($row['value'], $langsset);
                    $config_langs = $row;
                    break;
                }
            }
            ...
            if (!$config_langs){
                $config_langs['value'] = $langsset;
                $config_langs['name'] = "langs";
                $config_langs['type'] = "array";
                $config_langs['title'] = "array";
                $config[] = $config_langs;
            }
            ...
            $config_file = ROOT_PATH . "public" . DS . "templates" . DS . $temp_name . DS . $name . DS . "langs-{$langsname}.ini";
            //更新配置文件
     ...
     * @throws \think\Exception
     */
    public function langsedit($name = NULL)
    {

        $is_module = $this->request->param('module', 0, 'intval');//1是模块0是插件
        $langsname = $this->request->param('langsname');
        $temp_name = $is_module == 1 ? "modules" : "addons";
        if (!$name) $this->error('模板不存在');
        ...
        return $this->view->fetch('', ['langconfig' => $config]);
    }
